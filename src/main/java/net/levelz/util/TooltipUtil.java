            ...
            if (stack.getItem() instanceof BlockItem blockItem) {
                ...
                if (isCreative || !levelManager.hasRequiredBlockLevel(blockItem.getBlock())) {
                    if (LevelManager.BLOCK_RESTRICTIONS.containsKey(blockId)) {
                        ...
                        lines.add(Text.translatable("item.levelz.usable.tooltip"));
                        for (Map.Entry<Integer, Integer> entry : playerRestriction.getSkillLevelRestrictions().entrySet()) {
                            if (isCreative || levelManager.getSkillLevel(entry.getKey()) < entry.getValue()) {
                                lines.add(Text.translatable("item.levelz." + LevelManager.SKILLS.get(entry.getKey()).getKey() + ".tooltip", entry.getValue()).formatted(Formatting.RED));
                            }
                        }
                    }
                }
                if (isCreative || !levelManager.hasRequiredMiningLevel(blockItem.getBlock())) {
                    if (LevelManager.MINING_RESTRICTIONS.containsKey(blockId)) {
                        ...
                        lines.add(Text.translatable("item.levelz.mineable.tooltip"));
                        for (Map.Entry<Integer, Integer> entry : playerRestriction.getSkillLevelRestrictions().entrySet()) {
                            if (isCreative || levelManager.getSkillLevel(entry.getKey()) < entry.getValue()) {
                                lines.add(Text.translatable("item.levelz." + LevelManager.SKILLS.get(entry.getKey()).getKey() + ".tooltip", entry.getValue()).formatted(Formatting.RED));
                            }
                ...
                if (isCreative || !levelManager.hasRequiredCraftingLevel(stack.getItem())) {
                    if (LevelManager.CRAFTING_RESTRICTIONS.containsKey(itemId)) {
                        ...
                        lines.add(Text.translatable("item.levelz.craftable.tooltip"));
                        for (Map.Entry<Integer, Integer> entry : playerRestriction.getSkillLevelRestrictions().entrySet()) {
                            if (isCreative || levelManager.getSkillLevel(entry.getKey()) < entry.getValue()) {
                                lines.add(Text.translatable("item.levelz." + LevelManager.SKILLS.get(entry.getKey()).getKey() + ".tooltip", entry.getValue()).formatted(Formatting.RED));
                            }
...
//                    textList.add(Text.translatable("item.levelz.mineable.tooltip"));
                    for (Map.Entry<Integer, Integer> entry : levelManager.getRequiredMiningLevel(block).entrySet()) {
                        ...
                        textList.add(Text.translatable("item.levelz." + LevelManager.SKILLS.get(entry.getKey()).getKey() + ".tooltip", entry.getValue()).formatted(formatting));
                    }
    ...
    private static void renderTooltip(MinecraftClient client, DrawContext context, List<Text> textList, Identifier identifier, int x, int y) {
        int maxTextWidth = 0;
...
//        render(context, x - maxTextWidth / 2 - 3, y + 4, maxTextWidth, textList.size() * 10 + 8, 400, colorStart, colorTwo, colorThree);
        ...
        for (Text text : textList) {
            if (i == 9) {
                context.drawText(client.textRenderer, text, x - maxTextWidth / 2 + 20, y + i, 0xFFFFFF, false);
            } else {
//                context.drawText(client.textRenderer, text, x - maxTextWidth / 2, y + i + 5, 0xFFFFFF, false);
