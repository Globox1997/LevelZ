...
public class LevelzGui extends LightweightGuiDescription {
    ...
    public LevelzGui(MinecraftClient client) {
        ...
        if (!ConfigInit.CONFIG.useIndependentExp) {
            nextLevel = new WDynamicLabel(() -> String.format("XP %d / %d", playerStatsManager.getNonIndependentExperience(playerEntity), playerStatsManager.getNextLevelExperience()));
            ...
            nextLevel = new WDynamicLabel(
                    () -> "XP " + (int) (playerStatsManager.getLevelProgress(playerEntity) * playerStatsManager.getNextLevelExperience()) + " / " + playerStatsManager.getNextLevelExperience());
